...
public class DecodeurOCR {
	...
	public String decouperLeIemeChiffre(String entree, int i) {
		...
		return ligneCaractere1 + ligneCaractere2 + ligneCaractere3;
	}
	...
	public boolean estUnCompteValide(String entreedecodee) {
		int checksum = 0;
		for (int i = 0; i < entreedecodee.length(); i++) {
			int chiffre = Character.getNumericValue(entreedecodee.charAt(i));
			checksum += chiffre * (9 - i);
		}
